...
Future main() async {
  ...
  // for camera permission to add profile picture
  await Permission.camera.request();
  await Permission.photos.request();
  // await Permission.camera.request().isGranted; experimentally commented out on 19.3.22
...
class _MyAppState extends State<MyApp> {
  ...
  InAppWebViewGroupOptions options = InAppWebViewGroupOptions(
      ...
      // have to override application Agent because elseway Safari would override
      // the important mobileApp sign
      userAgent: "Safari Mobile mobileApp",
      // Agent info to identify appuser
      cacheEnabled: true,
      clearCache: false, // avoid to reload media files on every appstar
      ),
  ...
  Future<bool> _onWillPop(BuildContext context) async {
    // the ! is needed to ignore null cases
    if (await webViewController!.canGoBack()) {
      webViewController?.goBack();
      // deactivates the default back button handler which closes the app
      return Future.value(false);
    } else {
      // when set to true the default back button
      // handler is called and closes the app
  ...
  Future<void> setupInteractedMessage() async {
    ...
    // Stream listener
    // replacement for onLaunch and on Resume !
  ...
  // set the fcm token as secured cookie for the server
  ...
  // sets preferred device orientation if device is a smartphone
                          ...
                          child: Stack(
                            children: [
                              ...
                              Builder(builder: (BuildContext context) { return Container(
                                // Container is needed to color the ios bottom nav bar in #fb5137
                                color: const Color(0xfffb5137),
                                // Safearea needed to lift the WebView a bit above the ios navigation bar 
                                child: SafeArea(
                                  child: InAppWebView(
                                  key: webViewKey,
                                  initialUrlRequest:
                                  ...
                                  onWebViewCreated: (controller) async {
                                    ...
                                    // to allow only portraitup on Phones but all portraits on tablets
                                  ...
                                  onLoadStart: (controller, url) async {
                                    ...
                                    // or example domain/ no internet hmtl path
                                    ...
                                    if (redirectToBrowser){
                                      /* very important to call them with await on IOS;
                                      * if one of the two commands is missed, the return
                                      * to the app is weird with no internet error;
                                      * goes back two sites but thats acceptable */

                                      await webViewController?.stopLoading();
                                      await webViewController?.goBack();

                                      // !newurl.contains("example.com") || !newurl.contains("about:blank")
                                  ...
                                  onLoadStop: (controller, url) async {
                                    ...
                                    //final String? fetchToken = prefs.getString('messageingtoken');
                                  },
       ...
       d="m 103.20261,57.201487 14.91447,-18.803158 c 3.44276,-4.340396 9.70861,-5.063044 14.04901,-1.620288 4.34039,3.442755 5.06305,9.708614 1.62029,14.04901 l -14.91446,18.803156 c -3.44277,4.340396 -9.70862,5.063045 -14.04902,1.620289 -4.34039,-3.442756 -5.063037,-9.708614 -1.62029,-14.049009 z" />
        ...
        <p class="reloadtext">Reload</p>
                                  ...
                                  shouldOverrideUrlLoading: (controller, navigationAction) async {
                                    ...
                                    // stuff to redirect to browser if domain isnt
                                    // example.com is handled in onLoadStart
